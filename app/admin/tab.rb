	...
	form do |f|
	      ...
	      f.inputs  "Tab Details" do
	          ...
	          if f.object.id.nil?
				  ...
				  text_node( link_to "Open Content Editor (Mozilla FireFox only supported)", edit_content_path( :model_name => "tab", :id => f.object.id, :attribute => "content", :redirection_link => request.original_url ) )
			  end
	      end
	      f.actions
	end
